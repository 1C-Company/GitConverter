...
Процедура СформироватьОтчетПоВерсиямХранилища(Параметры) Экспорт
	...
	Если Параметры.Свойство("СформироватьОтчетПоВерсиямХранилища")
		...
		Если КодВозврата <> 0 Тогда
			...
			ПрочитатьЛогОшибки(НСтр("ru = 'Пакетная операция.Формирование отчета по хранилищу'"), Параметры, НСтр("ru = 'Ошибка выполнения пакетной операции.'"));
			
			ВызватьИсключение НСтр("ru = 'При формирования отчета по хранилищу конфигураций возникли ошибки (см. Журнал регистрации).'");
...
Процедура ОбновитьИнформационнуюБазуИзХранилища(Параметры) Экспорт
	...
	Если Параметры.Свойство("ОбновитьИнформационнуюБазуИзХранилища")
		...
		Если КодВозврата <> 0 Тогда
			...
			ПрочитатьЛогОшибки(НСтр("ru = 'Пакетная операция.Обновление из хранилища'"), Параметры, НСтр("ru = 'Ошибка выполнения пакетной операции.'"));
			
			ВызватьИсключение НСтр("ru = 'При обновлении из хранилища конфигураций возникли ошибки (см. Журнал регистрации).'");
...
Процедура ВыгрузитьКонфигурациюВФайлы(Параметры) Экспорт
	...
	Если Параметры.Свойство("ВыгрузитьКонфигурациюВФайлы")
		...
		Если КодВозврата <> 0 Тогда
			...
			ПрочитатьЛогОшибки(НСтр("ru = 'Пакетная операция.Выгрузка конфигурации в файлы'"), Параметры, НСтр("ru = 'Ошибка выполнения пакетной операции.'"));
...
// Читает файл лога и сообщает об ошибке в журнал регистрации
//
//	Параметры:
//		ИмяСобытия - Строка - Имя события журнала регистрации
//		Параметры - Структура - параметры запуска
//		Описание - Строка - описание события
//		Уровень - Строка - уровень журнала регистрации(по умол. Ошибка)
Процедура ПрочитатьЛогОшибки(ИмяСобытия, Параметры, Описание = "", Уровень = "Ошибка")
	Перем Лог;

	Если НЕ Параметры.Свойство("ИмяФайлаЛогов", "") Тогда 
		Возврат;
	КонецЕсли;
	...
	ДобавитьЗаписьВЖурналРегистрации(ИмяСобытия, Уровень, , Описание, ТекстЛога);

КонецПроцедуры //ПрочитатьЛогОшибки
