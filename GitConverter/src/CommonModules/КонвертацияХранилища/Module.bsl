...
Процедура ВыгрузитьВерсию(Знач ВерсияХранилища, Параметры) Экспорт
	...
	ПараметрыКомандыЗапуска1С(Параметры);
...
// Процедура дополняет параметры свойством "КомандаЗапуска1С"
// 
// Параметры:
// 	Параметры - Структура - параметры в которые вставляется команда запуска 1С
//
Процедура ПараметрыКомандыЗапуска1С(Параметры)
	
	КомандаЗапуска1СнаСервере = Константы.КомандаЗапуска1СвПакетномРежимеНаСервереLinux.Получить();
	Если ЗначениеЗаполнено(КомандаЗапуска1СнаСервере) Тогда
		Параметры.Вставить("КомандаЗапуска1С", КомандаЗапуска1СнаСервере);
	Иначе
		Параметры.Вставить("КомандаЗапуска1С", "%СтрокаКоманды%");
	КонецЕсли;
...
Процедура СоздатьФайловуюИнформационнуюБазу(Параметры) Экспорт
	...
	Если Параметры.Свойство("СоздатьФайловуюИнформационнуюБазу")
		...
		КодВозврата = Неопределено;
		СтрокаКоманды = СтрЗаменить(Параметры.КомандаЗапуска1С, "%СтрокаКоманды%", СтрокаКоманды);
