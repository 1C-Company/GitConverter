...
Процедура ВыполнитьКоммиты(Знач Хранилище) Экспорт
	...
	ИменаРеквизитов = Новый Структура("ВерсияВGit, Код, ЛокальныйКаталогGit, КаталогВыгрузкиВерсий, АдресРепозиторияGit, 
		...
		| КонвертироватьВФорматEDT, ДобавлятьМеткиСНомеромКонфигурации, АдресРепозиторияGit, ИмяВетки", 
		"ВерсияВGit", 
		"ВерсияВGit.Код");
...
// * ДобавлятьМеткиСНомеромКонфигурации - Булево - Признак добавления меток с версией конфигурации
...
Функция ВыполнитьДействияПередКоммитом(ПараметрыВерсии, РеквизитыХранилища)
	...
	Если ЗначениеЗаполнено(РеквизитыХранилища.АдресРепозиторияGit) Тогда
		...
		ТекстКоманды = "git pull --tags >> ""%ФайлЛога%"" 2>&1";
	...
	Если РеквизитыХранилища.ДобавлятьМеткиСНомеромКонфигурации Тогда
...
Функция ВыполнитьДействаияПослеКоммита(ПараметрыВерсии, РеквизитыХранилища)
	...
	Если ЗначениеЗаполнено(РеквизитыХранилища.АдресРепозиторияGit) Тогда
		...
		ТекстКоманды = "git push --progress --tags -u origin %ИмяВетки% >> ""%ФайлЛога%"" 2>&1";
...
// Добавляет метку с версией конфигурации, если в следующем коммите номер версии меняется. 
// 
// Параметры:
// 	ПараметрыВерсии - Структура - Параметры версии:
// * Ссылка - СправочникСсылка.ВерсииХранилища - Ссылка на помещенную версию в репозиторий
...
Функция ДобавитьМеткуСНомеромСборки(ПараметрыВерсии, РеквизитыХранилища)
	...
	ПутьНоваяВерсия = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПараметрыВерсии.ПутьКФайламПроектаВерсии 
	                                                + "Configuration") + "Configuration.mdo";
	ПутьТекущаяВерсия = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПараметрыВерсии.ПутьКФайламПроекта 
	                                                + "Configuration") + "Configuration.mdo";
	...
	МеткаСВерсией = "v" + НомерТекущейВерсии;
...
// Возвращаемое значение:
//  Строка - Версия конфигурации, Неопределено - если не удалось прочитать Configuration.mdo
// 
