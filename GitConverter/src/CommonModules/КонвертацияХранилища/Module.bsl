...
Процедура ВыполнитьКоммиты(Знач Хранилище) Экспорт
	...
	ИменаРеквизитов = Новый Структура("ВерсияВGit, Код, ЛокальныйКаталогGit, КаталогВыгрузкиВерсий, АдресРепозиторияGit, 
		...
		| КонвертироватьВФорматEDT", "ВерсияВGit", "ВерсияВGit.Код");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НЕ ФайлКомандыGit.Существует() ИЛИ ФайлКомандыGit.Размер() = 0
			...
			ЗаписатьФайлыКоммитаВерсии(ВерсияХранилища, ТекущаяВерсия, РеквизитыХранилища, ПараметрыВерсии, ИмяФайлаКомандыGit, ИмяФайлаЛога, ИмяФайлКомментария);
		...
		Иначе
			ФайлКонфигурации = ПараметрыВерсии.ПутьКФайламПроектаВерсии
				+ "Configuration.xml";
		КонецЕсли;
		Файл = Новый Файл(ФайлКонфигурации);

		Если Не Файл.Существует() И (НЕ РеквизитыХранилища.ВыгружатьИзменения
				ИЛИ НЕ ПараметрыВерсии.ВыгрузкаИзменений) Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Коммит.Отсутствуют файлы конфигурации';", Метаданные.ОсновнойЯзык.КодЯзыка), УровеньЖурналаРегистрации.Ошибка, Метаданные.Справочники.ВерсииХранилища, ПараметрыВерсии.Ссылка, НСтр("ru = 'Отсутствует файл Configuration.* в каталоге выгрузки версии';", Метаданные.ОсновнойЯзык.КодЯзыка));
			Возврат;
		КонецЕсли;

		Если ПараметрыВерсии.ВыгрузкаИзменений Тогда
			ИмяФайлаИндексов = ПараметрыВерсии.ПутьКФайламПроектаВерсии
				+ "DumpFilesIndex.txt";
			ТаблицаИндексов = ПрочитатьТаблицуИндексов(ИмяФайлаИндексов);
			ИмяФайлаИндексовПердыдущейВерсии = РеквизитыХранилища.ПутьКФайламПроекта
				+ "DumpFilesIndex.txt";
		...
		ЗапуститьПриложение(?(ЭтоWindowsСервер, "cmd /C ", "bash -c ")
			+ ТекстКоманды, РеквизитыХранилища.ЛокальныйКаталогGit, Истина);
		...
		Если Объект <> Неопределено
				И (Объект.Состояние <> Перечисления.СостоянияВерсии.ВерсияПомещена
...
Процедура СформироватьФайлыКоммитаВерсии(ВерсияХранилища) Экспорт
	...
	ИменаРеквизитов = Новый Структура("Код, ЛокальныйКаталогGit, КаталогВыгрузкиВРепозитории, КаталогВыгрузкиВерсий, 
		...
		| РазрешитьПомещатьАнонимноЕслиНеНайденПользователь, ВыгружатьИзменения, КонвертироватьВФорматEDT", "Код", "Владелец.ЛокальныйКаталогGit", "Владелец.КаталогВыгрузкиВРепозитории", "Владелец.КаталогВыгрузкиВерсий", "Владелец.АдресРепозиторияGit", "Владелец", "Владелец.ИмяВетки", "Владелец.МинимальноеКоличествоМетаданных", "Владелец.РазрешитьПомещатьАнонимноЕслиНеНайденПользователь", "Владелец.ВыгружатьИзменения", "Владелец.КонвертироватьВФорматEDT");
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда
		...
		ЗаписатьФайлыКоммитаВерсии(ВерсияХранилища, ПредыдущаяВерсия, РеквизитыХранилища, ПараметрыВерсии, ИмяФайлаКомандыGit, ИмяФайлаЛога, ИмяФайлКомментария);
...
Функция ПолучитьПараметрыОбработкиВерсии(Знач ВерсияХранилища) Экспорт
	...
	ИменаРеквизитов = Новый Структура("Хранилище, Код, Состояние, КаталогВременныхФайлов, ВыгрузкаИзменений", "Владелец");
	...
	Параметры.Вставить("РабочийКаталогEDT", ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(Параметры.КаталогВременныхФайлов
		+ "ws"));
...
Функция ИмяФайлаКомандыКоммитаВерсии(КодВерсии, КаталогВыгрузкиВерсий,
		ЭтоWindowsСервер) Экспорт
...
Функция ИмяФайлаКомментарияКоммитаВерсии(КодВерсии,
		КаталогВыгрузкиВерсий) Экспорт
...
Процедура ВыгрузитьВерсию(Знач ВерсияХранилища, Параметры) Экспорт
	...
	Параметры.Вставить("ФайлИзменений", Параметры.КаталогВременныхФайлов
		+ "diff.txt");
	Параметры.Вставить("ФайлСостоянияПредыдущейВерсии", Параметры.КаталогВременныхФайлов
		+ "PreviousState.xml");
			...
			Попытка
				...
				Если Состояние = Перечисления.СостоянияВерсии.ВерсияВыгружена
					...
					Если Параметры.КонвертироватьВФорматEDT Тогда
						ФайлСостоянияПредыдущейВерсии = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ВыборкаДетальныеЗаписи.КаталогВременныхФайлов)
							+ "ConfigDumpInfo.xml";
					Иначе
						КаталогФайловКонфигурации = КаталогФайловКонфигурации(ВыборкаДетальныеЗаписи.КаталогВременныхФайлов);
						ФайлСостоянияПредыдущейВерсии = КаталогФайловКонфигурации
							+ "ConfigDumpInfo.xml";
					КонецЕсли;
				...
				ИначеЕсли Состояние = Перечисления.СостоянияВерсии.ВерсияПомещена Тогда
					...
					ФайлСостоянияПредыдущейВерсии = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(КаталогВыгрузкиВРепозитории(РеквизитыХранилища))
						+ "ConfigDumpInfo.xml";
					ФайлСостоянияПредыдущейВерсии = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(РеквизитыХранилища.ЛокальныйКаталогGit)
	...
	Если Параметры.ВыгружатьИзменения Тогда
		...
		Если НайденныеФайлы.Количество() = 0 ИЛИ НайденныеФайлы.Количество() = 1
	...
	Если Параметры.КонвертироватьВФорматEDT Тогда
		ИмяФайлаИсточника = Параметры.КаталогФайловКонфигурации
			+ "ConfigDumpInfo.xml";
	...
	Если Параметры.ВыгружатьИзменения Тогда
		...
		Если Параметры.КонвертироватьВФорматEDT И Параметры.ВыгрузкаИзменений Тогда
			...
			Если ОбъектыДовыгрузки.Количество() > 0 Тогда
				...
				Параметры.ВыгружатьИзменения = Ложь;
				Параметры.Вставить("ФайлСпискаВыгрузки", Параметры.КаталогВременныхФайлов
					+ "dumplist.txt");
...
Процедура ИмпортироватьВерсиюВEDT(ВерсияХранилища, Параметры) Экспорт
	...
	Если Прав(Параметры.РабочийКаталогEDT, 1) = ПолучитьРазделительПути() Тогда
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "%РабочийКаталог%", Лев(Параметры.РабочийКаталогEDT, СтрДлина(Параметры.РабочийКаталогEDT)
			- 1));
	Иначе
	...
	Если Прав(Параметры.КаталогФайловКонфигурации, 1) = ПолучитьРазделительПути() Тогда
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "%КаталогФайловКонфигурации%", Лев(Параметры.КаталогФайловКонфигурации, СтрДлина(Параметры.КаталогФайловКонфигурации)
			- 1));
	Иначе
	...
	Если Прав(Параметры.КаталогПроекта, 1) = ПолучитьРазделительПути() Тогда
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "%КаталогПроектаEDT%", Лев(Параметры.КаталогПроекта, СтрДлина(Параметры.КаталогПроекта)
			- 1));
	Иначе
...
Процедура ПрочитатьСтруктуруВыгрузкиВерсии(ВерсияХранилища, Параметры) Экспорт
	...
	Для Каждого Файл Из Файлы Цикл
		...
		Если Файл.ЭтоФайл() Тогда
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				Если НЕ Параметры.КонвертироватьВФорматEDT
						И СтрЗаканчиваетсяНа(ПолноеИмя, ".xml") Тогда
				...
				ИначеЕсли Параметры.КонвертироватьВФорматEDT
						И СтрЗаканчиваетсяНа(ПолноеИмя, ".mdo") Тогда
					Отбор.Вставить("ФайлПолноеИмя", Файл.ПолноеИмя);
		...
		Если (КоличествоОбработано % 100) = 0 И Файл.ЭтоФайл() Тогда
			...
			ДлительныеОперации.СообщитьПрогресс(100 * КоличествоОбработано
				/ ?(КоличествоВсего = 0, 1, КоличествоВсего), СтрШаблон(ШаблонСообщения, КоличествоОбработано, КоличествоВсего, Формат(Скорость, "ЧДЦ=2; ЧГ=0"), Отбор.Уровень, Строка(ДеньГода(ОсталосьВремени)
				- 1) + Формат(ОсталосьВремени, "ДФ=' ''дн.'' HH:mm:ss'")));
		КонецЕсли;
	КонецЦикла;
	...
	Если Параметры.МинимальноеКоличествоМетаданных > 0
		...
		// Произошла проблема с выгрузкой - нужно переполучать версию
			...
			ДобавитьЗаписьВЛог(Параметры, НСтр("ru='Загрузка метаданных'"), "Произошла проблема при выгрузке или загрузке метаданных. Минимальное количество метаданных: "
	...
	Иначе
		Пока Пути.Количество() > 1 Цикл
			...
			Если Пути.Количество() = 2 Тогда
				...
				Пути.Удалить(Пути.ВГраница());
				Прервать;
			ИначеЕсли Пути.Количество() > 2
					И КвалифицированныеИмена.Получить(Пути[Пути.ВГраница()]) <> Неопределено Тогда
				...
				Прервать;
			ИначеЕсли Пути[0] = "Subsystems" И Пути[Пути.ВГраница()
					- 1] = "Subsystems" Тогда
				Пути.Добавить(Пути[Пути.ВГраница()] + ".mdo");
				ПолноеИмяРодителя = СтрСоединить(Пути, ПолучитьРазделительПути());
				Уровень = Пути.Количество();
...
Функция СформироватьUUIDПоРодителю(ПолноеИмя, ТаблицаИндексов)
	...
	Иначе
		Пока Пути.Количество() > 1 Цикл
			Путь = Пути[Пути.ВГраница()];
			Пути.Удалить(Пути.ВГраница());
			Если Путь = "Ext" И (Пути.Количество() < 3 ИЛИ Пути[Пути.ВГраница()
					- 1] <> "Commands") Тогда
				ПолноеИмяРодителя = СтрСоединить(Пути, ПолучитьРазделительПути())
					+ ".xml";
				Уровень = Пути.Количество();
...
Процедура СоздатьРепозиторийGit(Хранилище) Экспорт
	...
	ИменаРеквизитов = Новый Структура("ВерсияВGit, Код, ЛокальныйКаталогGit, КаталогВыгрузкиВерсий, АдресРепозиторияGit, 
		| КаталогВыгрузкиВРепозитории, ПользовательСервераGit, ПарольСервераGit", "ВерсияВGit", "ВерсияВGit.Код");
	РеквизитыХранилища = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Хранилище, ИменаРеквизитов);
	...
	Если ЭтоWindowsСервер Тогда
		ТекстКомандыУстановкиКаталога = "cd /D ""%ЛокальныйКаталогGit%"""
			+ ВыводЛога;
	Иначе
...
Процедура СформироватьОтчетПоВерсиямХранилища(Параметры) Экспорт
	...
	Если Параметры.Свойство("СформироватьОтчетПоВерсиямХранилища")
		...
		СтрокаПараметров = "DESIGNER /WA- /DisableStartupDialogs /L ru /VL ru
			...
			+ "
			| " + ?(УказыватьПустогоПользователя
			И ЗначениеЗаполнено(Параметры.ПарольПользователяИБ), "/P ""%ПарольПользователяИБ%"" ", "")
			+ "
...
Процедура ВыгрузитьКонфигурациюВФайлы(Параметры) Экспорт
	...
	Если Параметры.Свойство("ВыгрузитьКонфигурациюВФайлы")
		...
		Если Параметры.Свойство("ВыгружатьИзменения")
				И Параметры.ВыгружатьИзменения Тогда
		...
		ИначеЕсли Параметры.Свойство("ВыгружатьПоСписку")
				И Параметры.ВыгружатьПоСписку Тогда
...
Процедура ПрочитатьСвойстваОбъектаМетаданныхEDT(СтруктураФайла,
...
Функция ПолучитьСписокОбъектовДовыгрузки(ФайлИзменений, ИмяФайлаДампа)
	...
	Для индекс = 1 По ФайлИзменений.КоличествоСтрок() Цикл
		...
		Если Сегменты.Количество() > 0 И (Сегменты[0] = "Language"
				ИЛИ Сегменты[0] = "Configuration") Тогда
			Родитель = Сегменты[0];
	...
	Попытка
		...
		Если ЧтениеXML.Прочитать() И ЧтениеXML.Имя = "ConfigDumpInfo"
				И ЧтениеXML.Прочитать() И ЧтениеXML.Имя = "ConfigVersions" Тогда
			Пока ЧтениеXML.Прочитать() И ЧтениеXML.Имя = "Metadata" Цикл
				...
				Если ЗначениеЗаполнено(ИмяОбъекта) Тогда
					...
					Если Сегменты.Количество() > 2
							И ДочерниеОбъекты.Найти(Сегменты[2]) = Неопределено Тогда
						Продолжить;
					КонецЕсли;
					Изменения = ВыгруженныеИзменения.Получить(Родитель);
					Если Изменения <> Неопределено
							И Изменения.Найти(ИмяОбъекта) = Неопределено Тогда
...
Процедура ЗаписатьФайлыКоммитаВерсии(ВерсияХранилища, ПредыдущаяВерсия,
	...
	ИмяФайлаИндексов = РеквизитыВерсии.ПутьКФайламПроектаВерсии
		+ "DumpFilesIndex.txt";
	...
	Если ЗначениеЗаполнено(ПредыдущаяВерсия)
			И НЕ РеквизитыВерсии.ВыгрузкаИзменений Тогда
			// Явное переименование всех файлов, если переименовывается папка, 
		// необходимо учесть переименование подчиненных папок
		ИменаРеквизитов = Новый Структура("КаталогВременныхФайлов, Состояние, ЛокальныйКаталогGit, КаталогВыгрузкиВРепозитории, КонвертироватьВФорматEDT", "КаталогВременныхФайлов", "Состояние", "Владелец.ЛокальныйКаталогGit", "Владелец.КаталогВыгрузкиВРепозитории", "Владелец.КонвертироватьВФорматEDT");
		...
		Если РеквизитыПредыдущейВерсии.Состояние = Перечисления.СостоянияВерсии.МетаданныеЗагружены Тогда
			...
			ИмяФайлаИндексов = РеквизитыПредыдущейВерсии.КаталогФайловПроекта
				+ "DumpFilesIndex.txt";
		ИначеЕсли РеквизитыПредыдущейВерсии.Состояние = Перечисления.СостоянияВерсии.ВерсияПомещена Тогда
			ИмяФайлаИндексов = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(РеквизитыПредыдущейВерсии.ЛокальныйКаталогGit)
				+ КаталогВыгрузкиВРепозитории(РеквизитыПредыдущейВерсии)
				+ "DumpFilesIndex.txt";
		Иначе
			ВызватьИсключение Нстр("ru = 'Формирование файла невозможно для версии со статусом ""Начало коммита""';", Метаданные.ОсновнойЯзык.КодЯзыка);
		КонецЕсли;

		Файл = Новый Файл(ИмяФайлаИндексов);
		Если НЕ Файл.Существует()
				И РеквизитыПредыдущейВерсии.Состояние = Перечисления.СостоянияВерсии.МетаданныеЗагружены Тогда
			Возврат;
		КонецЕсли;
		...
		Если ЕстьУдаления Тогда
			ТекстКоманды = "git commit -F ""%ИмяФайлКомментария%"" --allow-empty-message --cleanup=verbatim"
				+ ВыводЛога;
	...
	Если НРег(МассивПутейИсточника[0]) = НРег(МассивПутейПриемника[0]) Тогда
		...
		// Если выгрузка только изменений - тогда обновляем существующие файлы
			...
			// Для изменений используется копирование с заменой
				...
				КонецЕсли;
				ТекстКоманды = "robocopy ""%Источник%"" ""%Приемник%"" /E /MOVE /NFL /NDL /NJH /NJS /NC /NS /NP"
					+ ВыводЛога;
	...
	ДетализацияКомментария = НСтр("ru = 'Версия хранилища №%НомерВерсии% от %Дата% %Автор%'");
	...
	Если Не ЗначениеЗаполнено(Комментарий) Тогда
		Комментарий = НСтр("ru = '<Комментарий не указан>'");
	КонецЕсли;

	Комментарий = СтрШаблон("%1
		|//------
		| %2", Комментарий, ДетализацияКомментария);
...
Функция КаталогВыгрузкиВРепозитории(РеквизитыХранилища) Экспорт
	...
	Если НЕ ПустаяСтрока(РеквизитыХранилища.КаталогВыгрузкиВРепозитории) Тогда
		ПрефиксКаталогаИсходников = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПрефиксКаталогаИсходников
			+ РеквизитыХранилища.КаталогВыгрузкиВРепозитории);
	КонецЕсли;
	ПрефиксКаталогаИсходников = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПрефиксКаталогаИсходников
		+ "src");
	...
	Пока СледующаяСтрока <> Неопределено Цикл
		...
		Если НЕ ПустаяСтрока(ТекущаяСтрока) Тогда
			...
			Если МассивСтрок.Количество() = 4 Тогда
				...
				Если Уровень = 0 ИЛИ ПустаяСтрока(UUID) ИЛИ ПустаяСтрока(ХешПолногоИмени)
						ИЛИ ПустаяСтрока(ПолноеИмя) Тогда
					Продолжить;
				КонецЕсли;
