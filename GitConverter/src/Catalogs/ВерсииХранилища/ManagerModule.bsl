			...
			Если ЗначениеЗаполнено(Значение) Тогда
				...
				Иначе
					Если ТипЗнч(ВерсияХранилища[Ключ]) = Тип("Массив") Тогда
						...
						ВерсияХранилища[ИмяТекущегоМассиваСтрок].Добавить(Значение);
					Иначе
						Если Ключ = "Версия" Тогда
							// удалим неразрывный пробел из номера версий, чтобы обеспечить безошибочную конвертацию
							// номера версии в число на системах с региональными настройками, отличными от российских
							ВерсияХранилища[Ключ] = СтрЗаменить(Значение, Символы.НПП, "");
						Иначе
