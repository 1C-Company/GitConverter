...
&НаКлиенте
Процедура ЗапуститьОбработкуОчереди(Команда)
	
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Возврат;
	КонецЕсли;
	
	ЗапуститьОбработкуОчередиНаСервере(Объект.Ссылка);
...
Процедура ЗапуститьОбработкуОчередиНаСервере(Очередь)
	
	ОбработкаОчередей.ЗапуститьОбработкуОчередиВФоне(Очередь);
