...
Процедура ЗаменитьВерсиюФормата(Параметры)
	...
	Если Файл.Существует() Тогда
		...
		Для Индекс = 1 По Мин(4, Текст.КоличествоСтрок()) Цикл
			...
			Если Позиция > 0 И Сред(СтрокаСВерсией, Позиция + 13, 1) = """" Тогда

				СтрокаСВерсией = Лев(СтрокаСВерсией, Позиция + 10) + ВерсияФормата
					+ Сред(СтрокаСВерсией, Позиция + 13, СтрДлина(СтрокаСВерсией) - Позиция
					- 13);
